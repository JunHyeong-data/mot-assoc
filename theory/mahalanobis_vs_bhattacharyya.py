# -*- coding: utf-8 -*-
"""
마할라노비스 vs 바타차야 -- 검출 불확실성이 지나갈 통로가 있는가.
...
세 가지 비용을 같은 장면에 대고 비교한다.
  ...
  (1) Maha-DeepSORT : d^2 = eps^T (S_t)^-1 eps,  S_t = H P H^T + R(h_i)
      ...
      => 검출 불확실성 통로가 구조적으로 없다. 이것이 현재 표준이다.
...
print("  즉 가림이 길어지면 바타차야의 검출 불확실성 통로도 닫힌다.")
print("  [1] 에서 가림이 커질수록 채널세기가 줄어든 것이 이 때문이다.")
print()
print("결론: 바타차야로 바꾸면 검출 불확실성 통로가 '생기기는 한다'.")
print("      마할라노비스(현행)에서는 통로 자체가 없다.")
print("      단 그 통로는 St 와 Sd 가 비슷한 크기일 때만 열려 있고,")
