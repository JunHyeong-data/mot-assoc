# -*- coding: utf-8 -*-
"""경로 후보들을 같은 자로 잰다 -- 검출 공분산이 할당에 도달하는가.

지금까지 이 저장소가 확인한 것:
  - 마할라노비스(트랙 공분산만) : Sigma_d 가 아예 안 들어간다. 경로 없음
  - 바타차야                    : 경로를 열지만 로그행렬식 항 하나뿐이고
...
print("  잔차 R 의 차이를 잰다. 0 이면 그 경로로는 검출 정보가 안 간다.")
print()
print("  %-22s %14s %14s %10s" % ("경로", "|R(개체별)|", "|dR| 개체-상수", "할당변경"))
print("  " + "-" * 66)
...
print()
print("  -> |dR| 이 기계정밀도(1e-16)면 그 경로는 Sigma_d 를 못 나른다.")
...
print()
print("  %-22s %s" % ("경로", "".join("%11s" % ("x%d" % s) for s in (1, 5, 20, 100))))
print("  " + "-" * 70)
...
print()
print("  -> 1.0 을 유지하면 가림에서도 경로가 열려 있다. 0 으로 가면 닫힌다.")
...
print("정리")
print("=" * 78)
print("  세 진단을 다 통과하는 경로만 검출 불확실성을 실제로 쓸 수 있다.")
print("  [1] 도달  [2] 가림에서 유지  [3] 역설 없음")
print()
print("  * 이건 합성 수치다. 실데이터에서 이득이 난다는 말이 아니라,")
print("    '어느 경로가 원리적으로 가능한가' 만 가린다.")
