...
| 낱말 | 출현 |
|---|---|
| `fuse` | **1회** — 그것도 관련 없는 문맥 (*detection-by-tracking* 절에서 예측 박스와 검출 박스를 합치는 이야기) |
...
> 고 적었다. **틀렸다.** LG-Track 이 정확히 그 경로를 쓰고 있고, **이득도 보고한다.**
>
> **읽지 않았으면 이미 점유된 경로를 '빈 칸' 이라고 주장할 뻔했다.**
...
캐스케이드 경로는 **비어 있지 않다.** 그러나 **더 나은 실험이 된다** —
이 프로젝트의 논지가 **소스 × 경로**이기 때문이다.

> **새 E2**: LG-Track 이 **이득을 보고한 그 경로**(위치 품질로 연관을 순서 지음)에
> **우리 소스(NMS σ, DFL σ)를 넣는다.** 경로는 고정, 소스만 간다.
>
> - **이득이 나오면** — 경로가 아니라 소스가 문제였다는 직접 증거
> - **이득이 안 나오면** — **작동하는 것이 확인된 경로에서도 σ 가 실패한다.**
>   이건 지금까지 중 **가장 강한 음성 결과**다. "경로를 잘못 골랐다" 는 반박이 죽는다
>
> **어느 쪽이든 이긴다. 그리고 원래 E2 보다 훨씬 강하다.**

한 가지 유의: LG-Track 의 `d_l`(예측 IoU 품질)과 우리 σ(산포)는 **다른 양**이다.
"같은 경로, 다른 소스" 라고 정확히 쓰고, **품질 ≠ 불확실성**임을 명시한다.
...
**이건 박스 크기 모형 그 자체다. 우리 실험의 기준선 `C` 다.**
검출기가 내는 불확실성이 아니라 **박스 크기 × 전역 상수 하나**.
...
지금까지 `C`(박스 크기 모형)는 "우리가 세운 기준선" 이었다. 이제 이렇게 된다:
...
| | 무엇을 R 에 넣었나 | 결과 |
|---|---|---|
| **UCMCTrack** (AAAI'24) | **박스 크기** (`σ_m·w`, `σ_m·h`) | SOTA |
| UTrack (ECCV'24 W) | **검출기 NMS σ** | 우리 재현 **−0.62** |
| UncertaintyTrack | **검출기 공분산** | 저자 보고 **−0.1** |

> **같은 경로(칼만 R), 다른 소스. 박스 크기를 쓴 쪽만 이긴다.**
...
**E2 의 정확한 위치**: LG-Track 과 **같은 쪽·같은 경로**, **다른 소스**
...
### ③ 성분별 절제(Table V, ByteTrack* / BDD100K val) — **우리 경로 성적표와 맞는다**
...
| **+ 박스 확장 (Relaxation)** | **34.8** | **45.1** | **+2.2 / +3.0 — 최대** |
| + **엔트로피 그리디 매칭** | 34.9 | 45.2 | **+0.1 — 최소** |
| + 칼만 R (전부) | **35.1** | 45.5 | +0.2 |

> **정정 (2026-08-21)**: 위 표는 그리디 행과 전체 행을 한 줄로 합쳐 적어
> 그리디의 기여를 **+0.2 로 잘못 적었다.** 실제 증분은 34.8 -> 34.9 = **+0.1**
> 이고, +0.2 는 거기에 **칼만 R 을 얹은** 몫이다 -- 즉 이 논문이 단독으로는
> **-0.1** 이라고 보고한 바로 그 성분이다. `uncertainty_related_work.md:28`
> 이 처음부터 +0.1 로 옳게 적고 있었는데 원고가 이 표를 따랐다.
...
> ### **독립적 확증이다**
> 우리 실험 6 은 순위형 자리(임계값)의 **오라클 상한이 +0.892** 라고 했다.
...
| 방법 | R 을 무엇으로 정하나 |
|---|---|
| **UCMCTrack** | **박스 크기** × 전역 σ_m ← **우리 기준선 `C`** |
...
순위형 경로(연관 순서)는 **LG-Track 과 UncertaintyTrack 이 이미 점유**했고,
UncertaintyTrack 의 절제가 **그 성분이 가장 작게 기여(+0.2)** 한다고 말한다.
**우리 실험 6 의 오라클 상한 +0.892 와 같은 방향이다.**

> **E2 는 새 실험이 아니라 이미 나온 두 증거의 확인이 된다.**
> 돌릴 값어치가 크게 줄었다. **대신 이 두 수치를 원고 8절에 인용한다** —
> *"순위형 자리에는 여지가 작다" 를 우리 오라클 상한과 남의 절제가 함께 말한다.*
...
```
Σ = diag( w²/4 ,  h²/4 )        박스의 내접 타원을 가우시안으로 본 것
```
...
> 쓰면 **리뷰어가 잡는다.** 정확한 서술은 —
> **"박스 크기에 비례하는 공분산이라는 같은 족이고, NWD 는 그중 (w,h) 비등방·
> 스케일 고정 판이다."**
>
