...
실측 세계는 W3 보다 **W2 에 가깝다.** 다만 눈금이 24배 어긋나 있으므로 (exp01)
W2 도 아니다 -- 굳이 말하면 'W2 의 방향에 W3 의 눈금'이다.
...
def make_world(world, seed=0, pair_consistent=True):
    ...
    **왜 이 스위치가 필요한가.** UTrack 규약상 트랙은 마지막으로 매칭된 검출의
    ...
    검출을 **같은 만큼** 키워 기하를 보존하는데, 뒤섞기 갈래(K3/K4)는 양쪽에
...
def sweep_world(frames, alphas, cap, verbose=True):
    """한 세계에서 다섯 갈래를 alpha 별로 돌리고 best alpha 의 차이를 낸다.
...
def main():
    ...
    print('   갈래가 정보에만 반응하면 크고, 짝 일관성 같은 부산물에 반응하면 0 이다.')
    print('')
    ...
    print('')
    print('   **판정 갈래는 K2 다.** K3/K4 는 진단으로만 쓴다.')
    print('   뒤섞기 갈래는 트랙과 검출에 서로 다른 순열을 걸어 정답쌍의 확장량이')
