...
## 이건 실험이 아니다. 사전 등록도 없다

트래커에 대해 아무 주장도 하지 않는다. **이미 잰 숫자들 위의 산술**이다.
그래서 사전 등록 없이 돈다. 대신 이 계산이 답하는 것은 실험만큼 중요하다:
...
실험 6 의 오라클 상한이 결합 +0.892 다. **효과가 작으면 필요 표본은 폭발한다.**
...
**우리는 지금까지 두 번째를 첫 번째 방법으로 주장해 왔다.** "판정 기준 0.3 안이면
...
def verify_combined():
    ...
    m = HOTA()
    ok = True
    print("%-16s %8s %8s   %s" % ("조건", "다시 잼", "기록", "차이"))
    print("-" * 56)
...
def main():
    ...
    print("=" * 96)
    print("실험이 아니다. 이미 잰 숫자들 위의 산술이다. 사전 등록 없음.")
    print()
    ...
    print()
    print("  **장면 간 산포가 판정 기준(%.1f)의 %.0f~%.0f 배다.**"
    ...
    print("  판정 기준 %.1f 과 비교하라." % BAND)

    # ---- 0.3 을 검출하려면 ----
    print()
    print("=" * 96)
    print("[3] 우월성 -- 판정 기준 %.1f HOTA 를 검출하려면 장면이 몇 개 필요한가" % BAND)
    print("=" * 96)
    ...
    print("=" * 96)
    print("  '판정 기준 안이면 차이 없음' 은 동등성 주장이다. 점추정이 아니라")
