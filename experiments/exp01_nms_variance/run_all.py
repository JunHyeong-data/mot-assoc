# -*- coding: utf-8 -*-
"""실험 1 의 세 갈래를 전부 다시 낸다.

여태 갈래별 실행 명령이 어디에도 적혀 있지 않았다. 환경변수 조합을 손으로
쳐서 돌렸고, 그래서 무엇이 어떤 설정으로 나온 숫자인지 npz 파일명에만 남았다.
**재현이 안 되는 상태였다.** 이 스크립트가 그 구멍을 메운다.

  (빈 TAG)  우리 검출기, 사전 선언 설정. 판정용 본 실행
...
사용법:
    ...
    python experiments/exp01_nms_variance/run_all.py base m60   # 갈래 골라서
"""
import os
import subprocess
import sys
...
# 갈래 -> (TAG, 환경변수, 시퀀스별 프레임 수)
# 본 실행만 MOT17-02 를 전체(299)로 돌린다. 관문 판정을 처음 낸 실행이 그랬다.
ARMS = {
...
def main():
    ...
    if bad:
        sys.exit("모르는 갈래: %s (가능: %s)" % (bad, list(ARMS)))

    t0 = time.time()
    for arm in want:
        tag, env_extra, nframe = ARMS[arm]
        print("=" * 72)
        print("갈래 %s   TAG=%r   %s" % (arm, tag, env_extra["EXP01_MODEL"]))
        print("=" * 72)
        for seq in SEQS:
