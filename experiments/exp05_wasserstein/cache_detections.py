...
**왜 캐시하나.** 갈래마다 검출기를 다시 돌리면 (a) 시간이 갈래 수만큼 들고
(b) 검출이 갈래마다 미세하게 달라질 여지가 생긴다. 한 번 돌려 캐시하고
트래커만 재생하면 **갈래 사이에서 검출이 비트 단위로 같다.** 갈리는 것이
비용함수 하나뿐이 된다. exp00 의 `sweep.py` 가 쓴 것과 같은 수법이다.

저장하는 것 (프레임당):
    xyxy   (n,4)  원본 좌표 검출 상자
    conf   (n,)   점수
...
그 가로채기는 상자 재구성이 NMS 출력과 6e-05 px 안에서 일치하는 것으로 검산됐다.
