# -*- coding: utf-8 -*-
"""실험 5 [2단계] -- 와서스타인 비용함수. 조건별 Sigma 만 다르다.
...
우리 경우 St, Sd 가 **전부 대각**이다 (DFL 은 변마다 독립 softmax, 박스크기 모형은
...
`C` 는 데이터셋 상수다. **채택률을 기준선과 맞추도록 보정한다** (사전 등록 함정 3).
보정 안 하면 경로 효과와 임계값 효과가 섞인다.
...
**Sigma_t 를 조건과 무관하게 칼만 P 로 고정하는 것이 핵심 통제다** (함정 2).
조건마다 다른 궤적을 타면 통제가 깨지고, 동시에 exp03 의 짝 일관성 교란도
원천 차단된다 -- 트랙이 검출의 Sigma 를 물려받지 않기 때문이다.
`nwd_exact` 만 그 통제를 깨는데, 그건 **문헌 재현용 참조점**이지 판정 조건이 아니다.
"""
import numpy as np
...
def w2_matrix_norm(t_xyxy, d_xyxy, t_var, d_var):
    """**크기 정규화** W^2 (실험 5b. 사전 등록은 PREREG-norm.md).

    쌍마다 박스 크기로 좌표를 나눈 뒤 같은 W^2 을 쓴다. 아핀 사상
    ...
    **한쪽 박스로 나누지 않고 기하평균을 쓴다.** 한쪽이면 비용이 비대칭이 되고
...
def size_var(xyxy):
    """NWD 의 박스-가우시안: Sigma = diag(w^2, h^2)/4."""
...
def match_scale(var, target_tr):
    """전역 배율로 E[tr Sigma] 를 target 에 맞춘다 (사전 등록 함정 1).

    DFL 은 0.26배 과대, NMS 는 80배 과소다. 날것으로 넣으면 조건 간 차이가
