...
아래로 들어오는 쌍의 비율이 달라질 수 있고, 그러면 갈래 간 HOTA 차이가
...
2단계는 `get_dists` 를 안 쓰고 `iou_distance` 를 직접 부르므로 갈래와 무관하고,
3단계(unconfirmed, `:362`, `thresh=0.7`)는 갈래가 갈리지만 규모가 작고
...
def main():
    ...
    print("갈래 간 채택률이 비슷하면 HOTA 차이는 임계값 효과가 아니다.")
    print()
    hdr = "%-14s%10s%12s%14s%10s"
    print(hdr % ("갈래", "C", "매칭 수", "최적쌍 상한", "채택률"))
    print("-" * 74)
