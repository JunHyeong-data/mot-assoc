...
`run.py -nms`:
    ...
    합친 AUC          0.5355   (사전 등록 기준 <0.55 -> "정보 없음")
    시퀀스별 중앙값     0.6275   (사전 등록 기준 >=0.60 -> "짚는다")
...
`exp01` 이 **σ 스케일이 시퀀스마다 62~1840배 갈린다**고 했다. 시퀀스를 합쳐
하나의 순위로 세우면 **스케일 차이가 순위를 지배**해서 시퀀스 안의 판별력이
씻긴다. 표준적인 층화 문제다.

## 보는 것 — **전부 탐색. 사전 등록 밖이다**

    [a] 시퀀스 안에서 순위정규화한 뒤 합쳐 AUC   (스케일 차이 제거)
    [b] 시퀀스별 AUC 의 오류 수 가중 평균         (층화 추정)
    [c] MOT17-04 가 왜 0.897 인가                (오류가 141건뿐이다)
    [d] 같은 것을 DFL 로도                        (DFL 은 안 갈리는가)

> **사전 등록한 주 평가지표는 [1] 합친 AUC 하나이고 그 값은 0.5355 다.**
...
def one(tag):
    ...
    print("  사전 등록 [1] 합친 AUC              %.4f" % pooled)
...
def main():
    print("탐색 -- 사전 등록 밖이다. **판정을 바꾸지 않는다**")
    print()
    res = {}
    for tag in ("nms", "dfl"):
        r = one(tag)
