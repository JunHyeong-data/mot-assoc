# -*- coding: utf-8 -*-
"""**그림 1 — 추정 방식 × 적용 경로 지도.** 문헌이 무엇을 어디에 넣고 무엇을 얻었나.

## 왜 이 그림인가

이 연구의 논지가 **추정 방식 × 적용 경로**다. 두 축을 한 장에 놓으면 문헌의
성패가 **추정 방식을 따라 갈린다**는 것이 보인다 -- 같은 경로(칼만 R)에서도
...
## 자료 -- 전부 **원문을 직접 읽고** 채웠다 (`notes/primary_reading.md`)
  ...
  UncertaintyTrack 칼만 R **-0.1** / 박스확장 **+2.2** / 엔트로피 순서 **+0.2**

**우리 것은 기울임체다.** 어느 칸을 우리가 채웠는지 보여야 한다.

**거리 함수 행은 2026-08-18 에 넣었다** -- 원고 2.3 이 적용 경로 넷 중 하나로
세는데 지도에 그 행이 없었다.
...
CELLS = {
    ...
    # **한 칸에 두 이야기가 있다.** UncTrack 의 +2.2 는 박스확장 전체의 이득이고,
