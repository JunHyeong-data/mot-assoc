...
> **연구 노트 저장소입니다.** 결과뿐 아니라 **틀렸던 것과 왜 틀렸는지**를 함께 공개합니다.
> 지금까지 **17건의 결론을 철회**했고, 하나도 지우지 않았습니다 → [철회 기록](#철회-기록)
...
| **4** | 그럼 왜 안 되나? | **σ는 "어느 검출이 나쁜가"를 말하지 "어느 짝이 틀렸는가"를 말하지 않는다** (AUC **0.457**) | 실험 15·18 |

> **핵심 대조.** 같은 자리에 **박스 크기**를 넣으면 헝가리안이 실제로 푸는 형태(행 안 비교)에서
...
그 비용에 **검출이 얼마나 믿을 만한지**는 부분적으로만 들어간다.
...
score는 *"이 상자에 사람이 있는가"* 이고, 이 연구가 묻는 것은 *"이 상자가 어디 있는지를 얼마나 아는가"* 다.
...
† **개입 불성립** — 이 조건은 확장량을 키울수록 채택률이 오히려 떨어져 확장이 문을 여는 개입이 되지 못한다.
값은 적되 판정에서 뺀다.

**읽는 법.** 유효한 다섯 칸 중 넷이 음수이고, 양수인 한 칸은 검출기 σ가 아니라 **대조군**이다.
`+0.074`는 설계 해상도(3.31 HOTA)의 1/45라 이득의 증거가 아니라 *"박스를 키우는 것 자체는 공짜"* 로 읽는다.
**따라서 교락은 해소되지 않았고, 이를 결과가 아니라 한계로 유지한다.**

### 자를 바꾸면 박스 크기가 살아난다

채택된 쌍 전체의 주변부 AUC로 재면 박스 크기도 **0.466**으로 무용해 보인다.
그러나 헝가리안이 실제로 푸는 형태 — **같은 호출 안에서 정답과 오답을 짝지어 비교** — 로 재면:

| 신호 | 주변부 AUC | **행 안 정확도** |
|---|--:|--:|
| **박스 크기** | 0.466 | **64.5%** (95% CI 0.623–0.667) |
| 검출기 σ (NMS) | 0.457 | 0.4415 |
| 검출기 σ (DFL) | 0.536 | 0.4720 |

> **주변부 지표로 선별했다면 문헌에서 가장 성공적인 신호 계열을 시작 전에 기각했을 것이다.**
> 다만 이 점검은 **필요조건이지 충분조건이 아니다** — 박스 크기는 통과하고도 두 경로에서 기준선을 못 넘었다.

<details>
<summary><b>이론 — 통로별로 갈린다 (펼치기)</b></summary>
...
> **그런데 실증에서 닫혔다.** 실제 트래커에 넣으니 맨 IoU보다 **−8.90 HOTA**다.
> **"통로가 열려 있다"는 "성능이 오른다"가 아니다** — 이 저장소의 상시 교훈이다.
...
실데이터에서도 확인됐다 (실험 11) — 가산은 쌍이 달라진 12건이 **전부 동점**이고 최적성이 0건 깨졌다.
...
| **2** | **사전 등록한 절차를 안 지킨 값으로는 판정도 철회도 하지 않는다** — 이게 가장 비싼 교훈이다 |
...
| **6** | 판정 기준은 **설계 해상도 안에서** 정한다. n=7에서 검출 가능한 최소 효과는 **3.31 HOTA**다 |
...
| ~~정규화하면 오히려 나빠진다~~ | **사전 선언한 눈금을 안 지킨 값으로 판정했다.** 제대로 재니 부호가 뒤집혔다 |
