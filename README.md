...
| **미결** | 경로와 추정 방식의 교란 — 격자 한 칸이 양수라 해소되지 않았다 · 게이팅의 공정한 대조군을 구성하는 방법 |
...
### 2. 경로가 나쁜가, 추정 방식이 나쁜가 — 격자로 갈랐다

하나의 재생 파이프라인 안에서 두 경로 × 세 추정 방식. 기준선 HOTA 61.002 대비.
...
`+0.074`는 최소 검출 가능 효과(3.31 HOTA)의 1/45라 이득의 증거가 아니라 *"박스를 키우는 것 자체는 공짜"* 로 읽는다.
사전 등록한 규칙은 유효한 칸이 모두 음수일 것을 요구했으므로 **교란은 해소되지 않았고,
이를 결과가 아니라 한계로 유지한다.**
...
**부호가 갈린다.** 범위가 8.735 HOTA로 최소 검출 가능 효과의 두 배가 넘는다.
...
> `+0.191`을 *"DFL이 이겼다"* 로 읽지 않는다. 최소 검출 가능 효과의 1/17이고 시퀀스별 부호도 세지 않았다.
