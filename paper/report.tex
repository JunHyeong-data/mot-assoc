...
게이팅(박스 확장) & 기준선 & $-4.33$ & 확장량 일치 대조군을 따로 둠 \\
...
\caption{왼쪽: 시퀀스별 기준선 HOTA와 오라클 상한까지의 여지. 여지가 장면에 따라 13배 갈린다. 오른쪽: 결합 기준 $\Delta$HOTA. 오라클 둘과 카메라 움직임 보상만 0보다 크고, \textbf{$\sigma$를 주입한 네 경로는 전부 0보다 작다.} 세 값의 비교 조건은 표~\ref{tab:channels}와 같다.}
\label{fig:ceiling}
\end{figure}

\textbf{개선 여지는 $+3.122$ HOTA로 존재한다.} 우리가 시도한 네 경로는 모두 음의 값이었으므로, ``딸 것이 없었다''는 설명은 기각된다. 4.3절에서 언급한 대로 이 상한은 오탐과 미검출을 수정하지 않으므로 보수적인 값이다. 그림~\ref{fig:ceiling}에 이 상한과 우리가 시도한 경로들을 나란히 두었다.
