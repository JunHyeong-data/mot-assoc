...
적혀 있지 않았다. 게다가 오류가 `ModuleNotFoundError: No module named 'tracker'` 로만 나와서
무엇을 해야 하는지 알 수 없었다. 아래를 먼저 채운다.

| 필요한 것 | 왜 git에 없나 | 채우는 법 |
|---|---|---|
| `external/UTrack` | 외부 저장소 — 벤더링된 TrackEval 지표 클래스를 여기서 가져온다 | `git clone https://github.com/DLR-MI/UTrack.git external/UTrack` |
...
| `data/exp01` (σ·오차 npz) | 용량 | `python experiments/exp01_nms_variance/run_all.py` |
| `data/exp06` (임계값 그리드) | 용량 | `python experiments/exp06_levers/grid.py` |

꾸러미는 `theory/` 에 `numpy`·`scipy` 면 되고, `experiments/` 는 `ultralytics`·`torch`·
`opencv-python` 이 더 필요하다.
